...
export async function POST(req: NextRequest) {
  try {
    const { joinCode, userEmail, userUid, acceptedAgreement } = await req.json();
    if (!joinCode || (!userEmail && !userUid)) return NextResponse.json({ error: 'joinCode and userEmail required' }, { status: 400 });
    if (!acceptedAgreement) return NextResponse.json({ error: 'You must accept the roommate agreement before joining' }, { status: 400 });

    try {
      const res = await fetch(`${API_BASE}/groups/join`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ joinCode: joinCode.toUpperCase().trim(), userEmail, userUid: userUid || userEmail, acceptedAgreement: true }),
      });
