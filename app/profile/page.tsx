    ...
    <div style={{ maxWidth:900, margin:'0 auto', padding:'40px 24px 80px' }}>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'340px 1fr', gap:24, alignItems:'start' }}>
        ...
        <div style={{ background:'var(--bg-card)', border:'1px solid var(--border)', borderRadius:18, padding:'32px 28px', boxShadow:'var(--shadow-sm)', textAlign:'center' }}>
          <div style={{ width:80, height:80, borderRadius:'50%', background:'var(--accent)', display:'flex', alignItems:'center', justifyContent:'center', fontSize:28, fontWeight:800, color:'#fff', fontFamily:'Syne,serif', margin:'0 auto 20px' }}>
            {initials}
          </div>
          ...
          {editing ? (
            <div style={{ textAlign:'left', marginBottom:20 }}>
              ...
              <div style={{ display:'flex', gap:8, marginTop:12 }}>
                <button onClick={handleSave} disabled={saving} style={{ flex:1, display:'flex', alignItems:'center', justifyContent:'center', gap:6, padding:'10px', borderRadius:9, background:saving?'var(--border)':'var(--accent)', color:saving?'var(--text-muted)':'#fff', border:'none', cursor:saving?'not-allowed':'pointer', fontFamily:'Syne,serif', fontWeight:700, fontSize:13 }}>
                  <Save size={13}/>{saving?'Saving…':'Save'}
                </button>
                <button onClick={()=>setEditing(false)} style={{ flex:1, padding:'10px', borderRadius:9, border:'1px solid var(--border)', background:'var(--bg-subtle)', cursor:'pointer', fontFamily:'Syne,serif', fontWeight:700, fontSize:13, color:'var(--text-secondary)' }}>Cancel</button>
              </div>
            </div>
          ) : (
            <>
              ...
              <button onClick={()=>setEditing(true)} style={{ display:'inline-flex', alignItems:'center', gap:6, padding:'8px 18px', borderRadius:9, border:'1.5px solid var(--border)', background:'var(--bg-subtle)', color:'var(--text-secondary)', cursor:'pointer', fontSize:13, fontWeight:700, fontFamily:'Syne,serif', marginBottom:20 }}>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:10, marginBottom:20 }}>
            {[
              ...
              <div key={s.label} style={{ padding:'12px 8px', background:'var(--bg-subtle)', borderRadius:10, border:'1px solid var(--border)' }}>
                ...
                <p style={{ fontSize:18, fontWeight:700, fontFamily:'Syne,serif', color:'var(--text-primary)' }}>{s.val}</p>
                <p style={{ fontSize:11, color:'var(--text-muted)', fontFamily:'Syne,serif', textTransform:'uppercase', letterSpacing:'0.06em' }}>{s.label}</p>
              </div>
            ))}
          </div>

          <div style={{ display:'flex', flexDirection:'column', gap:8 }}>
            <Link href="/my-listings" style={{ display:'flex', alignItems:'center', justifyContent:'center', gap:7, padding:'11px', borderRadius:10, background:'var(--accent)', color:'#fff', fontWeight:700, fontFamily:'Syne,serif', fontSize:14 }}>
              <BookMarked size={14}/> My Listings
            </Link>
            <Link href="/household" style={{ display:'flex', alignItems:'center', justifyContent:'center', gap:7, padding:'11px', borderRadius:10, border:'1.5px solid var(--border)', color:'var(--text-primary)', fontWeight:700, fontFamily:'Syne,serif', fontSize:14, background:'var(--bg-subtle)' }}>
              My Household
            </Link>
            <button onClick={logout} style={{ display:'flex', alignItems:'center', justifyContent:'center', gap:7, padding:'11px', borderRadius:10, border:'1px solid var(--border)', background:'transparent', color:'var(--danger)', fontWeight:700, fontFamily:'Syne,serif', fontSize:14, cursor:'pointer' }}>
              Sign Out
            </button>
          </div>
        </div>
        ...
        <div>
          <div style={{ background:'var(--bg-card)', border:'1px solid var(--border)', borderRadius:14, padding:'22px 24px', marginBottom:20, boxShadow:'var(--shadow-xs)' }}>
            <h3 style={{ fontSize:16, fontWeight:700, marginBottom:16, fontFamily:'Syne,serif', display:'flex', alignItems:'center', gap:8 }}>
            ...
            <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:14 }}>
              {[
                ...
                <div key={s.label} style={{ padding:'14px 16px', background:'var(--bg-subtle)', borderRadius:10, border:'1px solid var(--border)', borderLeft:`3px solid ${s.color}` }}>
                  <p style={{ fontSize:11, color:'var(--text-muted)', fontFamily:'Syne,serif', fontWeight:700, textTransform:'uppercase', letterSpacing:'0.07em', marginBottom:6 }}>{s.label}</p>
                  <p style={{ fontSize:15, fontWeight:700, color:s.color, fontFamily:'Syne,serif' }}>{s.val}</p>
                </div>
              ))}
            </div>
          </div>
          ...
          <div style={{ background:'var(--success-light)', border:'1px solid var(--success)', borderRadius:14, padding:'20px 24px' }}>
            ...
            <p style={{ fontSize:13, color:'var(--text-secondary)', lineHeight:1.65 }}>
              ✅ <strong>Firebase Authentication</strong> — Real email/password and Google Sign-In enabled<br/>
